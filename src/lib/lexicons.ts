...
export interface LexiconRenderer {
  ...
  getAuthor: (record: ATProtoRecord) => { name: string; handle: string; avatar?: string };
...
const bskyPostLexicon: LexiconRenderer = {
  ...
  getLink: (record) => `https://bsky.app/profile/${record.author.handle}/post/${record.uri.split('/').pop()}`,
...
const bskyLikeLexicon: LexiconRenderer = {
  ...
  getLinkText: (record) => record.resolvedSubject?.uri ? 'View Original Post on Bluesky →' : 'View Profile on Bluesky →',
...
const whitewindBlogLexicon: LexiconRenderer = {
  ...
  getContent: (record) => record.value.content || record.value.text || 'No content',
  getAuthor: (record) => ({
    name: record.author.displayName || record.author.handle,
    handle: record.author.handle,
    avatar: record.author.avatar
  ...
  getTags: (record) => record.value.tags || [],
  getTimestamp: (record) => record.indexedAt || record.value.createdAt,
  getLink: (record) => `https://whtwnd.com/${record.author.handle}/${record.uri.split('/').pop()}`,
...
export const getLexiconForRecord = (record: ATProtoRecord): LexiconRenderer | undefined => {
...
export const getLexiconsByView = (view: 'compact' | 'expanded' | 'full'): LexiconRenderer[] => {
...
export const getActivityLexicons = (): LexiconRenderer[] => {
  return Array.from(lexiconRegistry.values()).filter(lexicon => lexicon.showInActivity);
};

export const getBlogLexicons = (): LexiconRenderer[] => {
  return Array.from(lexiconRegistry.values()).filter(lexicon => lexicon.showInBlog);
};
...
export const getRecordContent = (record: ATProtoRecord, maxLength?: number): string => {
  const lexicon = getLexiconForRecord(record);
...
export const getRecordLink = (record: ATProtoRecord): string => {
  const lexicon = getLexiconForRecord(record);
  return lexicon?.getLink(record) || `https://bsky.app/profile/${record.author.handle}`;
};
